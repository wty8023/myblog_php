...
//        $query = $this -> db -> get_where("t_user",array('name'=>'lisi'));
    ...
    public function get_message_by_receiver($user_id)
    {
//        $query = $this->db->get_where('t_message', array('receiver' => $user_id));
//        return $query->result();
        $this->db->select('*');
        $this->db->from('t_message m');
        $this->db->join('t_user u','m.sender=u.user_id');
        $this->db->where('m.receiver',$user_id);
        return $this->db->get()->result();
    }
    public function get_message_by_sender($user_id)
    {
        $this->db->select('*');
        $this->db->from('t_message m');
        $this->db->join('t_user u','m.receiver=u.user_id');
